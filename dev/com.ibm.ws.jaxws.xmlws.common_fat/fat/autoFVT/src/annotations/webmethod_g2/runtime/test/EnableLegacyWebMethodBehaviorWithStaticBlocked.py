                ...
                if( jpdName != 'Control' ):
                    if( jpdName == 'Servant' ):
                        ...
                        for jvm in jvmps:
                            ...
                            # useWSFEP61ScanPolicy=true enables legacy annotation scanning behavior in v7 forwards.
                            # and is needed to keep wsfp tests from breaking.
                            jvmArgs = AdminConfig.showAttribute(jvm, 'genericJvmArguments')
                            print 'jvmargs before change is %s' % (jvmArgs)
                            jvmArgs = jvmArgs + " -Djaxws.runtime.legacyWebMethod=true  -Djaxws.runtime.restrictStaticWebmethod=true"
