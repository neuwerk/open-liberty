...
# call wsgen with all arguments and bad input parameter, make sure we don't get
...
sh $WAS_BIN/jaxws/wsgen$ext -keep  -d $BUILD_WORK -s $BUILD_WORK \
  -r $TEST_DIR -wsdl[soap1.1] -verbose -extension \
  -cp $BUILD_CLASSES:. ThereIsNoSuchSEI \
