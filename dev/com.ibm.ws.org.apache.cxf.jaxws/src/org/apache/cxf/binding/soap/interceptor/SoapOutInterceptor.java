...
public class SoapOutInterceptor extends AbstractSoapInterceptor {
    ...
    private static final Logger LOG = LogUtils.getL7dLogger(SoapOutInterceptor.class);
    ...
    private void writeSoapEnvelopeStart(final SoapMessage message) {
        ...
        try {            
            ...
            LOG.info("soapPrefix: " + soapPrefix + " soapVersion.getNamespace()" + soapVersion.getNamespace().toString());
            ...
            if (message.hasAdditionalEnvNs()) {
                ...
                xtw.setPrefix(soapPrefix, soapVersion.getNamespace());
                LOG.info("xtw " + xtw.getClass());
                xtw.writeStartElement(soapPrefix, 
                                      soapVersion.getEnvelope().getLocalPart(),
                                      soapVersion.getNamespace());
                LOG.info("soapPrefix " + soapPrefix + " soapVersion.getEnvelope().getLocalPart() " + soapVersion.getEnvelope().getLocalPart() + " soapVersion.getNamespace() " + soapVersion.getNamespace());
                String s2 = xtw.getPrefix(soapVersion.getNamespace());
                LOG.info("StringUtils.isEmpty(s2) " + StringUtils.isEmpty(s2) + " soapPrefix.equals(s2)" + soapPrefix.equals(s2));
