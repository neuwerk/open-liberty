...
public class HttpConduitPropertiesTest {
    ...
    // Timeout to be used to wait for copying ibm-ws-bnd.xml to complete before invoking Web Service
    private static int BND_FILE_COPY_TIMEOUT = 1000;
    ...
    public static void setup() throws Exception {
        ...
        // Get default log once - to be used across tests
        defualtLog = server.getDefaultLogFile();
    }
    ...
    public void testPropertiesForMatchedServiceRef() throws Exception {
        ...
        // Add a pause incase file hasn't yet finished copying that can be increased as needed
        if (!server.fileExistsInLibertyServerRoot(server.getServerRoot() + "dropins/httpConduitProperties.war/WEB-INF/ibm-ws-bnd.xml"))
            Thread.sleep(BND_FILE_COPY_TIMEOUT);
    ...
    public void testPropertiesForMatchedServiceRefAndPort() throws Exception {
        ...
        // use mark to check for servlet readiness
        server.waitForStringInLog("CWWKZ0001I.*httpConduitProperties");
        server.waitForStringInLog("SRVE0253I");
